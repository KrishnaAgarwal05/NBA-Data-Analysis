...
reader = spark.read
path = 'file:/C:\\Hardik\\Studies\\Sem II\\Big Data\\Project\\nba-players-stats\\'
...
tuu = spark.createDataFrame(test,["A","B"])
...
def drop_Duplicates(column_list,data_file):
    non_duplicates_df = data_file.dropDuplicates(subset=column_list)   
...
def nba_champions_stats():
    ...
    nba_winners = nba_winners.select(col('Champion').alias('State'), 
                                                         col('Titles').alias('Titles'))

    nba_runners = nba_champions_df.groupby('Opponent').agg(F.count('Opponent').alias('Runners_Up'))
    nba_runners = nba_runners.select(col('Opponent').alias('State'), 
...
if __name__ == "__main__":
    ...
    #collegewise_stats_df.show(10)
    #statewise_stats_df.show(10)
    #player_position_collegewise_df.show(10)
    
    t4 = time()
    
    nba_championship_stats_df = nba_champions_stats()
    #nba_championship_stats_df.show(10)
    
    t5 = time()
    
    time_stats = [["Part 1",(t1-start_time)*1000],["Part 2",(t2-t1)*1000],["Part 3",(t3-t2)*1000],
                  ["Part 4",(t4-t3)*1000],["Part 5",(t5-t4)*1000]]
    
    cSchema = StructType([StructField("Part", StringType())
                      ,StructField("Time in millisecond", FloatType())])
    
    time_stats_df = spark.createDataFrame(time_stats,cSchema)
    time_stats_df.show()
